...
app.post('/', async (req, res) => {
    ...
    const response = await openai.createCompletion({
        ...
        prompt:  
        `Going to give text to GPT-3 in multiple chunks of text. Do not respond with anything until user asks question, which will be asked in its own input without any other text. It also be denoted as the user question by saying "user question:
        ${message}`,
        max_tokens: 500,
        temperature: 0,
    });
